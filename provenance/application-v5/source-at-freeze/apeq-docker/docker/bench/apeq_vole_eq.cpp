...
ProtocolResult run_protocol(const Config& c, const Inputs& in) {
    ...
    const auto session_bind_sent = chl.bytesSent();
    const auto session_bind_recv = chl.bytesReceived();
    timer.start();
    ...
    const uint64_t base_ot_sent = chl.bytesSent() - session_bind_sent;
    const uint64_t base_ot_recv = chl.bytesReceived() - session_bind_recv;
    ...
    res.setup_bytes_sent = chl.bytesSent();
    res.setup_bytes_recv = chl.bytesReceived();
    ...
    if (c.party == 2) {
        ...
        for (uint64_t i = 0; i < c.batch; ++i) {
            ...
            expected_hashes[i] = vole_digest(sid, c.bits, c.batch, i, receiver_a[i]);
        }
        ...
        for (uint64_t i = 0; i < c.batch; ++i) {
            ...
            hashes[i] = vole_digest(sid, c.bits, c.batch, i, adjusted_share);
        }

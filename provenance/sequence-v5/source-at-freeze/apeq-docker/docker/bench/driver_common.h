...
struct Config {
    ...
    uint32_t sequence_requests = 0; // E3: 0=ordinary mode; 1=cold; 20=warm
...
inline Config parse_args(int argc, char** argv) {
    ...
    for (int i = 1; i < argc; ++i) {
        ...
        else if (a == "--sequence-requests") { need(i); c.sequence_requests = std::stoul(argv[++i]); }
        else if (a == "--request-index") { need(i); c.request_index = std::stoul(argv[++i]); }
    ...
    if (c.sequence_requests) {
        if (c.private_input.empty() || c.sequence_requests > 20 || c.request_index >= 20 ||
            c.sequence_requests > 20 - c.request_index ||
            (c.sequence_requests > 1 && c.request_index != 0) ||
            c.protocol != "apeq" || (c.backend != "bit_ot" && c.backend != "ferret_vole"))
            die("sequence mode requires private-file APEQ bit-OT/VOLE and at most 20 requests");
    } else if (c.request_index) die("request index requires sequence mode");
...
inline int application_main(const Config& c) {
    ...
    metrics << std::setprecision(17)
        ...
        << "\"rep\":" << c.rep << ",\n"
        << "\"sequence_requests\":" << c.sequence_requests << ",\n"
...
inline int driver_main(int argc, char** argv) {
    ...
    if (!c.private_input.empty()) {
        try {
            if (!c.sequence_requests) return application_main(c);
            // In sequence mode the private-input and output arguments name
            // role-local directories. The public manifest is the same for all
            // requests; each private file has a fresh fixed input vector.
            for (uint32_t offset = 0; offset < c.sequence_requests; ++offset) {
                Config request = c;
                request.request_index = c.request_index + offset;
                const auto suffix = std::to_string(request.request_index);
                request.run_id = c.run_id + "-q" + suffix;
                request.private_input = c.private_input + "/request-" + suffix + ".csv";
                request.out = c.out + "/request-" + suffix + ".json";
                if (c.party == 2)
                    request.equality_output = c.equality_output + "/request-" + suffix + ".csv";
                application_main(request);
            }
            return 0;
        }
        catch (const std::exception& e) {

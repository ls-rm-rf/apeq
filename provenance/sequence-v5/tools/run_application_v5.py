"""Fixed E1 job. Preparation is portable; execution uses the WSL Docker daemon.
...
def prepare():
    ...
    design = dict(frozen_utc=utc(), experiment='E1', runs=240, repeats=10,
                  ...
                  E3='deferred until E1 handoff; same-connection API/security audit required',
...
def run():
    ...
    try:
        ...
        write_json(JOB/'completion.json', dict(completed_utc=utc(), runs=240,
                   ...
                   next_step='Await user notification; E3 and manuscript analysis have not started.'))
        update(state='COMPLETED', stage='awaiting-user', current='E1 validated; notify Codex to continue', current_run_id=None)

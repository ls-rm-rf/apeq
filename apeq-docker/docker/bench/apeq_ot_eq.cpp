...
ProtocolResult run_protocol(const Config& c, const Inputs& in) {
    ...
    ProtocolResult res;
    auto socket = request_socket(c);
    const auto sent_before = socket.bytesSent();
    const auto recv_before = socket.bytesReceived();
    ...
    res.setup_bytes_sent = socket.bytesSent() - sent_before;
    res.setup_bytes_recv = socket.bytesReceived() - recv_before;
    ...
    res.bytes_sent = socket.bytesSent() - sent_before;
    res.bytes_recv = socket.bytesReceived() - recv_before;
    ...
    if (c.sequence_requests)
        res.note += " fresh_crypto_setup_per_request opened_connections=" +

...
$jobDirectory = Join-Path (Split-Path -Parent $PSScriptRoot) 'experiments\application-v5'
...
do {
    ...
    if (Test-Path -LiteralPath $statePath) {
        ...
        Write-Host ("[{0}] {1} | {2} | E1 {3}/{4} ({5}%) | preflight {6}/8 | elapsed {7}" -f `
        ...
        if ($state.state -eq 'COMPLETED') {
            Write-Host 'E1 collection and per-pair correctness checks are complete. Run the independent analysis next.' -ForegroundColor Green
